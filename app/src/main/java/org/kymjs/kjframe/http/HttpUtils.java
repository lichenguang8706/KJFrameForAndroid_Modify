...
/**
 * HTTP工具类
 * 将HttpUtils中的所有HttpResponse替换成自定义的KJHttpResponse（by 我是无名氏）
 * @author 张涛
 *
 */
public class HttpUtils {
    ...
    public static String getCharset(KJHttpResponse response) {
        ...
        if (header != null) {
            String contentType = header.get(KJHTTP.CONTENT_TYPE);
    ...
    public static String getHeader(KJHttpResponse response, String key) {
        String headerValue = response.getHeaders().get(key);
        return headerValue == null ? null : headerValue;
    }
