...
public class HttpConnectStack implements HttpStack {
    ...
    /**
     * 通过HttpURLConnection发起请求后的所有返回数据不再封装到HttpClient中的HttpResponse方法中，因为到了Android6.0之后，HttpClient已经被删除了
     * 相应的HttpResponse也不存在了，所以只能通过自定义的KJHttpResponse替代原有的封装方式。（by 我是无名氏）
     */
    @Override
    public KJHttpResponse performRequest(Request<?> request,
        ...
        kjHttpResponse = new KJHttpResponse();
        kjHttpResponse.setStatusCode(responseCode);
        kjHttpResponse.setResponseMessage(connection.getResponseMessage());
        ...
        kjHttpResponse.setContent(inputStream);
        kjHttpResponse.setContentEncoding(connection.getContentEncoding());
        kjHttpResponse.setContentLength(connection.getContentLength());
        kjHttpResponse.setContentType(connection.getContentType());
